"""
Volatility model classes used by the ECB stance forecasting study.
=================================================================
...
USAGE
-----
    from ecb_vol_forecasting.models import GARCH11, NAGarchHt
...
def _loglik_studentt(eps, sigma_sq, nu):
    ...
    log_const = gammaln(half_nu_p1) - gammaln(0.5 * nu) - 0.5 * np.log(np.pi * (nu - 2.0))
    log_lik = log_const - 0.5 * np.log(sigma_sq) - half_nu_p1 * np.log1p(z2 / (nu - 2.0))
    return float(log_lik.sum())


def _loglik_normal(eps, sigma_sq):
    return float((-0.5 * np.log(2.0 * np.pi * sigma_sq) - 0.5 * eps * eps / sigma_sq).sum())
...
class ModelBase:
    ...
    def conditional_variance(self):
        ...
        if self.dates is not None:
            return pd.Series(self._sigma_sq_in, index=self.dates, name=f"sigma2_{self.name}")
...
class _NAGarchBase(ModelBase):
    ...
    def forecast_variance(self, h=1):
        ...
        scale = self._scaling_factor_forecast(self._theta_opt[3 : 3 + self._theta_news_dim])
    ...
    def fit(self, theta_init=None, n_restarts=3, warm_start=None):
        ...
        candidates.append(self._theta_init() if theta_init is None else np.asarray(theta_init))
...
class NAGarchAsym(_NAGarchBase):
    ...
    def _scaling_factor(self, theta_news):
        ...
        if a + b < 0.5 or a + b > 2.0:
            return np.full(self.nobs, np.nan)
        return a + 0.5 * b * (np.tanh(0.5 * kappa * self.P) - np.tanh(0.5 * gamma * self.N))
    ...
    def _scaling_factor_forecast(self, theta_news):
        ...
        gamma = float(np.exp(theta_news[3]))
        return a + 0.5 * b * (
            np.tanh(0.5 * kappa * self.P_last) - np.tanh(0.5 * gamma * self.N_last)
        )
...
class NAGarchHt(_NAGarchBase):
    ...
    def __init__(self, returns, P, N, roll_vol, long_run_mean, c, dist="studentst"):
        super().__init__(returns, dist=dist)
        P_arr = np.asarray(P.values if hasattr(P, "values") else P, dtype=float)
        N_arr = np.asarray(N.values if hasattr(N, "values") else N, dtype=float)
        rv_arr = np.asarray(
            roll_vol.values if hasattr(roll_vol, "values") else roll_vol, dtype=float
        )
        ...
        self.c = float(c)
        self.H = np.where(
            np.isfinite(rv_lag), (rv_lag > self.c * self.long_run_mean).astype(float), 0.0
        )
    ...
    def _scaling_factor(self, theta_news):
        ...
        f = a + 0.5 * b_eff * (np.tanh(0.5 * kappa * self.P) - np.tanh(0.5 * gamma * self.N))
        return f
